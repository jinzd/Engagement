...
import {Container, Spinner, Button, Collapse, Row, Col, Card, CardBody, CardText} from 'reactstrap'
...
class EngagementGraph extends React.Component{
  ...
  getGraphData = (callback)=>{
    axios({
      ...
      mainGraph.forEach(point => {
        tempGraphDataMain[this.timecodeFixed + 's']=Math.floor(point['eng'] * 100)
  ...
  render()
  {
    const {backgroundColor,backgroundColorLigher, color, spinner} = this.state.activeTheme
    return(
      <>
        {/* <Container > */}
        <Row>
          <Col style={{'padding':10}}>
          <Card body style={{'display':this.state.isLoading ? 'block' : 'none', 'backgroundColor':backgroundColor, 'color':color}}>
            <CardBody className='justify-content-center'>
              <Spinner color={spinner} ></Spinner>
            </CardBody>
          </Card>
          <Card body style={{'display':this.state.isLoading ? 'none' : 'block', 'backgroundColor':backgroundColor, 'color':color, 'paddingLeft':5, 'paddingRight':5, 'paddingTop':0}}>
          <CardBody>
            <Row>
              <Col>
                <CardText style={{'fontWeight':'lighter', 'textAlign':'left'}}>{this.state.type} | {this.state.duration} ({this.state.date}, {this.state.time})</CardText>
                <CardText style={{'fontWeight':'lighter', 'fontSize':'1.7vh', 'textAlign':'left'}}></CardText>
                <Button color='info' onClick={()=>{this.setState({isOpen:!this.state.isOpen})}} style={{'fontWeight':'bold'}}>{this.state.title}</Button> 
                <Row style={{'paddingTop':20, 'paddingBottom':20}}>
                  <Col>
                  <Collapse isOpen={this.state.isOpen}>
                  <Card style={{'backgroundColor':backgroundColorLigher, 'color':color, 'borderWidth':0, 'padding':10}} body>
              ...
              <Col lg='4'>
                ...
                <CardBody>
                  <CardText style={{'fontWeight':'bold', 'textAlign':'left'}}>Expressions %</CardText>
                  <ColumnChart colors={["#ffb700"]} height={80} data={this.state.highestEmotionsPieChartData}></ColumnChart>
                </CardBody>
                </Card>
              </Col>
            </Row>
            <Row>
            <Col>
              <CardText style={{'fontWeight':'bold', 'textAlign':'left'}}>Engagement Index</CardText>
              <AreaChart dataset={{borderWidth: 3}}  colors={["#6840ed"]}height={100} data={this.state.mainGraphData}/>
              <Row>
                <Col>
                  <CardText style={{'fontWeight':'bold', 'textAlign':'left'}}>Attendance</CardText>
                  <ColumnChart colors={["#40edae"]} height={100} data = {this.state.faceCountGraphData}></ColumnChart>
                </Col>
